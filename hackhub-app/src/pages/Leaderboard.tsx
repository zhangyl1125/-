...
export function Leaderboard({ hackathonId: propHackathonId }: LeaderboardProps = {}): ReactElement {
  ...
  const summary = useQuery({ queryKey: ['score-summary', hackathonId], queryFn: () => JudgingService.getScoreSummary(hackathonId!), enabled: Boolean(hackathonId && canManage), staleTime: 30000 })
  ...
  const details = useQuery({ queryKey: ['all-judge-scores', hackathonId], queryFn: () => JudgingService.getAllScores(hackathonId!), enabled: Boolean(hackathonId && canManage) })
  ...
  const judgeGroups = [...new Set(selectedScores.map((score) => score.judgeId))]
  ...
  return <Container size={1320} w="100%" miw={0} style={{ maxWidth: 1320 }} py={{ base: 'md', md: 'xl' }} className="dp-page"><Stack gap="xl" miw={0}>
    <Group justify="space-between" align="flex-end">
      <div><Group gap="xs" mb="md"><ActionIcon variant="subtle" aria-label="Back to award event" onClick={() => navigate(`/hackathons/${hackathonId}`)}><IconArrowLeft size={18} /></ActionIcon><Text className="dp-section-label" mb={0}>Committee view · Live totals</Text></Group>
